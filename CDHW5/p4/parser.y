...
program			: ID
			{
			  ...
			  if (strcmp(fileName, $1)) {
				fprintf (stdout, "<Error> found in Line %d: program ID at the beginning is inconsistent with filename\n", linenum);
			  }
			}
			  MK_SEMICOLON
			  program_body
			...
			{
			  if (strcmp($1, $6)) {
                  fprintf (stdout, "<Error> found in Line %d: program ID at the end is inconsistent with the one at the beginning\n", linenum);
              }
			  if (strcmp(fileName, $6)) {
				  fprintf (stdout, "<Error> found in Line %d: program ID at the end is inconsistent with filename\n", linenum);
			  }
			  // dump symbol table
			  if( Opt_D == 1 )
...
func_decl		: ID MK_LPAREN opt_param_list
			  ...
			  MK_RPAREN opt_type 
			{
			  ...
			  if( paramError == __TRUE ) {
			  	printf("<Error> found in Line %d: param(s) with several error\n", linenum);
					...
					printf("<Error> found in Line %d: return type of a function cannot be array type\n", linenum);
				} else {
			...
			{
			  if( strcmp($1,$11) ) {
				fprintf( stdout, "<Error> found in Line %d: function name at the end of declaration is inconsistent with the one at the beginning\n", linenum );
			  }
			  funcReturn = 0;
			}
			;
...
for_stmt		: FOR ID
			  ...
			  END DO
			{
			  ...
			  if( Opt_D == 1 ) {
                  // Note this is just for table dump
                  struct SymNode *newNode;
                  struct PType *pType = createPType( INTEGER_t );
                  newNode = createVarNode( $2, scope, pType );
