...
void verifyLoopParam(int lo, int hi)
{
    if (lo < 0 || hi < 0)
    {
        fprintf(stdout, "<Error> found in Line %d: lower bound or upper bound of loop parameter < 0\n", linenum);
    }
    else if (lo > hi)
    {
        fprintf(stdout, "<Error> found in Line %d: loop parameter's lower bound > upper bound\n", linenum);
    }
}
...
SEMTYPE verifyArrayIndex(struct expr_sem *expr)
{
    ...
    {
        fprintf(stdout, "<Error> found in Line %d: array index is not an integer\n", linenum);
        result = ERROR_t;
    }
    else
    {
...
__BOOLEAN verifyExistence(struct SymTable *table, struct expr_sem *expr, int scope, __BOOLEAN isAssignmentLHS)
{
    ...
    { // deference and verify, if pass, setup PType field in expr_sem
        ...
        {
            fprintf(stdout, "<Error> found in Line %d: '%s' is the program\n", linenum, node->name);
            expr->pType = createPType(ERROR_t);
            result = __FALSE;
        }
        else if (node->category == FUNCTION_t)
        {
            fprintf(stdout, "<Error> found in Line %d: '%s' is a function\n", linenum, node->name);
            expr->pType = createPType(ERROR_t);
            result = __FALSE;
        }
...
void verifyUnaryMinus(struct expr_sem *expr)
{
    ...
    {
        fprintf(stdout, "<Error> found in Line %d: operand of unary - is not integer/real type\n", linenum);
        expr->isDeref = __TRUE;
...
void verifyUnaryNOT(struct expr_sem *expr)
{
    ...
    {
        fprintf(stdout, "<Error> found in Line %d: operand of operator 'not' is not boolean type\n", linenum);
        expr->pType->type = ERROR_t;
    }
    else
...
void verifyModOp(struct expr_sem *op1, struct expr_sem *op2)
{
    ...
    {
        fprintf(stdout, "<Error> found in Line %d: one of the operands of operator 'mod' is not integer type\n", linenum);
        op1->pType->type = ERROR_t;
    }
    else
    { // pass verify
...
void verifyArithmeticOp(struct expr_sem *op1, OPERATOR operator, struct expr_sem *op2)
{
    ...
    { // fail verify, dump error message
        ...
        printOperator(operator);
        fprintf(stdout, "' are not both integer type or both real type\n");
        op1->pType->type = ERROR_t;
    }
}
...
void verifyAndOrOp(struct expr_sem *op1, OPERATOR operator, struct expr_sem *op2)
{
    ...
    else if ((op1->pType->type) != BOOLEAN_t || (op2->pType->type) != BOOLEAN_t)
    {
        fprintf(stdout, "<Error> found in Line %d: one of the operands of operator '", linenum);
        printOperator(operator);
        fprintf(stdout, "' is not boolean type\n");
        op1->pType->type = ERROR_t;
    }
    else
    { // pass verification, result is boolean!
...
struct expr_sem *verifyFuncInvoke(const char *id, struct expr_sem *exprList, struct SymTable *table, int scope)
{
    ...
    { // check parameters...
        if (node->attribute->formalParam->paramNum == 0)
        {
            if (exprList != NULL)
            {
                fprintf(stdout, "<Error> found in Line %d: too many arguments passed to function '%s'\n", linenum, node->name);
                result->pType = createPType(ERROR_t);
            }
            else
            {
        ...
        else
        {
            ...
            else
            {
                if (listPtr != NULL)
                {
                    fprintf(stdout, "<Error> found in Line %d: too few arguments passed to function '%s'\n", linenum, node->name);
                    result->pType = createPType(ERROR_t);
                }
...
void verifyScalarExpr(struct expr_sem *expr, const char *str)
{
    ...
    {
        fprintf(stdout, "<Error> found in Line %d: expression part of %s statement is array type\n", linenum, str);
    }
}
...
void verifyBooleanExpr(struct expr_sem *expr, const char *str)
{
    if (expr->pType->type == ERROR_t)
    {
        fprintf(stdout, "<Error> found in Line %d: expression part of %s statement is not boolean type\n", linenum, str);
    }
    else if (expr->pType->dim)
    {
        fprintf(stdout, "<Error> found in Line %d: expression part of %s statement is array type\n", linenum, str);
        ...
        else
        {
            for (idPtr = (parPtr->idlist); idPtr != NULL; idPtr = (idPtr->next))
            {
                ...
                { // without error, insert into symbol table
                    newNode = createParamNode(idPtr->value, scope, parPtr->pType, var_no++);
...
void insertLoopVarIntoTable(struct SymTable *table, const char *id)
{
    ...
    if (nodePtr != 0)
    {
        ...
        pushLoopVar(table, createLoopVarNode(id, var_no++));
    }
    else
    {
        pushLoopVar(table, createLoopVarNode(id, var_no++));
