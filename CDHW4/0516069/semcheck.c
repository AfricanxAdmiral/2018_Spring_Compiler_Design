...
struct ConstAttr *createConstAttr( SEMTYPE type, void *value )
{
	...
	switch( type ) {
	 case INTEGER_t:
		...
		break;
	 case REAL_t:
		result->value.realVal = *(double*)value;
		if( *(double*)value < 0.0 )
			result->hasMinus = __TRUE;
		break;
	 case STRING_t:
...
void verifyArrayDim(struct PType *pType, int lo, int hi)
{
	...
	if(lo < 0 || hi < 0) isPass = __FALSE;
	else if(lo >= hi) isPass = __FALSE;
	if(isPass == __FALSE) pType->isError = __TRUE;
}
...
void verifyArrayType(struct idNode_sem *ids, struct PType *pType)
{
	...
	if(pType->isError == __TRUE) {
		printf("<Error> found in Line %d: invalid dimension declaration for array ", linenum);
		printf("%s", ids->value);
		for(ptr = ids->next; ptr != 0; ptr = ptr->next) printf(", %s", ptr->value);
		printf("\n");
	}
}
...
__BOOLEAN verifyExistence( struct SymTable *table, struct expr_sem *expr, int scope, __BOOLEAN isAssignmentLHS )
{
	...
	node = lookupLoopVar(table, expr->varRef->id);
	if(node == 0)
		node = lookupSymbol(table, expr->varRef->id, scope, __FALSE);
	if(node == 0) 
	{
		printf("<Error> found in Line %d: '%s' is not declared\n", linenum, expr->varRef->id);
	...
	else
	{
		...
		{
			printf("<Error> found in Line %d: '%s' is program\n", linenum, node->name);
			expr->pType = createPType( ERROR_t );
			result = __FALSE;
		}
		//else if( node->category == FUNCTION_t )
		//{
		//	printf("<Error> found in Line %d: '%s' is function\n", linenum, node->name);
		//	expr->pType = createPType( ERROR_t );
		//	result = __FALSE;
		//}
...
struct expr_sem *verifyFuncInvoke( const char *id, struct expr_sem *exprList, struct SymTable *table, int scope )
{
	...
	struct SymNode *node = 0;
	node = lookupSymbol( table, id, 0, __FALSE );
	...
	{
		printf("<Error> at Line %d: symbol '%s' is not a function\n", linenum, id );
		result->pType = createPType( ERROR_t );
	}
	else
	{
...
void verifyUnaryMinus( struct expr_sem *expr )
{
	...
	{
		printf("<Error> found in Line %d: operand of unary - is not integer/real\n", linenum );
		expr->isDeref = __TRUE;
...
{
	if( lo<0 || hi<0 ) {
		printf("<Error> found in Line %d: lower or upper bound of loop parameter < 0\n", linenum );
	}
	else if( lo >= hi ) {
...
SEMTYPE verifyArrayIndex( struct expr_sem *expr )
{
	SEMTYPE result;
	if( expr->isDeref == __FALSE ) {
		printf("<Error> found in Line %d: array index is not integer\n", linenum );
		result = ERROR_t;
	}
	else if( expr->pType->isArray == __TRUE ) {
		printf("<Error> found in Line %d: array index cannot be array type\n", linenum );
		result = ERROR_t;
	}
...
void verifyUnaryNOT( struct expr_sem *expr )
{
	...
	if( expr->pType->dimNum != 0 ) {
		printf("<Error> found in Line %d: operand of 'not' cannot be array_type\n", linenum );
		expr->pType->type = ERROR_t;
	}
	else if( expr->pType->type != BOOLEAN_t ) {
		printf("<Error> found in Line %d: operand of 'not' is not boolean\n", linenum );
		expr->pType->type = ERROR_t;
	}
	else {
...
void verifyAssignmentTypeMatch( struct expr_sem *LHS, struct expr_sem *RHS )
{
	...
	if( (LHS->pType->type)==ERROR_t || (RHS->pType->type)==ERROR_t )
		printf("<Error> found in Line %d: error type in RHS of assignment\n", linenum );
	...
	//if( LHS->varRef->dimNum || RHS->varRef->dimNum )
	//	printf("<Error> found in Line %d: array assignment is not allowed\n", linenum );
	if( misMatch == __TRUE )
	{
		printf("<Error> found in Line %d: type mismatch, LHS= ", linenum );
...
void verifyModOp( struct expr_sem *op1, struct expr_sem *op2 )
{
	if( (op1->pType->type)==ERROR_t || (op2->pType->type)==ERROR_t ) {
		...
		printf("<Error> found in Line %d: adjacent operator 'mod", linenum);
		printf("' and '" );
		...
		printf("<Error> found in Line %d: operand(s) between 'mod' are array type\n", linenum );
		op1->pType->type = ERROR_t;
	}
	else if( (op1->pType->type)!=INTEGER_t || (op2->pType->type)!=INTEGER_t ) {
		printf("<Error> found in Line %d: operand(s) between 'mod' are not integer\n", linenum );
		op1->pType->type = ERROR_t;
	}
	else
	{
...
void verifyArithmeticOp( struct expr_sem *op1, OPERATOR operator, struct expr_sem *op2 )
{
	...
	if( (op1->pType->type)==ERROR_t || (op2->pType->type)==ERROR_t ) {
		...
		printf("<Error> found in Line %d: adjacent operator '", linenum);
		...
		printf("<Error> found in Line %d: operand(s) between '", linenum);
		printOperator( operator );
		printf("' are array_type\n" );
		op1->pType->type = ERROR_t;
	}
	else {
		if( op1->pType->type==STRING_t && op2->pType->type==STRING_t ) {
			if( operator == ADD_t ) {
				op1->pType->type = STRING_t;
			}
			else {
				printf("Error at Line %d: operand(s) between '", linenum);
				printOperator( operator );
				printf("' are string type\n" );
			}
		}
...
{
	if( expr->pType->dim )
		printf("<Error> found in Line %d: %s statement's operand is array type\n", linenum, str );
}
...
void verifyBooleanExpr( struct expr_sem *expr, const char *str )
{
	if( expr->pType->dim )
		...
		printf("<Error> found in Line %d: %s statement's operand is not boolean type\n", linenum, str );
	}
}
...
{
	if( funcReturn == 0 ) {
		printf("<Error> found in Line %d: program cannot be returned\n", linenum );
	}
	else if( funcReturn->type == VOID_t ) {
		printf("<Error> found in Line %d: void function cannot be returned\n", linenum );
	}
		...
		printf("<Error> found in Line %d: return array dimension mismatch\n", linenum );
	}
	else {
		...
		for( returnDim=(funcReturn->dim), exprDim=(expr->pType->dim), i=0 ; returnDim!=0 ; returnDim=(returnDim->next), exprDim=(exprDim->next), ++i ) {
			if( returnDim->size != exprDim->size ) {
				printf("<Error> found in Line %d: return dimension %d's size mismatch\n", linenum, i );
			}
		}
	}
}
